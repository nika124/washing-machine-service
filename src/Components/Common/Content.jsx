  ...
  return (
    <div className={`grid gap-4 sm:gap-5 ${columns}`}>
      {items.map((item) => (
        <article
          key={item.path}
          className="group surface-card flex min-w-0 flex-col p-5 transition duration-300 hover:-translate-y-1 hover:border-blue-200 hover:shadow-[0_22px_55px_-28px_rgba(37,99,235,0.35)] sm:p-6"
        >
          <span
            aria-hidden="true"
            className="mb-5 flex h-11 w-11 items-center justify-center rounded-2xl bg-blue-50 text-blue-600 transition group-hover:bg-blue-600 group-hover:text-white"
          >
            <svg
              viewBox="0 0 24 24"
              className="h-5 w-5"
          ...
          <p className="text-lg font-bold text-slate-950">
            <Link
              className="focus-ring rounded hover:text-blue-700"
              to={item.path}
            >
          ...
          {item.text && (
            <p className="mt-3 flex-1 leading-7 text-slate-600">{item.text}</p>
          )}
          <Link
            className="mt-5 inline-flex items-center font-bold text-blue-700 hover:text-blue-900"
            to={item.path}
          >
            სრულად ნახვა{" "}
            <span
  ...
  return (
    <ul className="grid gap-3 sm:grid-cols-2 sm:gap-4">
      {items.map((item) => (
        <li
          key={item}
          className="surface-card flex items-start gap-3 p-4 leading-7 text-slate-700 sm:p-5"
        >
          <span
            aria-hidden="true"
            className="mt-0.5 flex h-6 w-6 shrink-0 items-center justify-center rounded-full bg-blue-600 text-xs font-black text-white"
          >
            ✓
          </span>
          <span>{item}</span>
  ...
  return (
    <ol className="grid gap-4 md:grid-cols-2 lg:grid-cols-4">
      {items.map((item, index) => (
        <li
          key={item}
          className="surface-card relative overflow-hidden p-5 leading-7 text-slate-700 sm:p-6"
        >
  ...
  return (
    <div className="space-y-3">
      {items.map((item) => (
        <details
          key={item.question}
          className="group surface-card p-5 open:border-blue-200 sm:p-6"
        >
          <summary className="flex min-h-7 cursor-pointer list-none items-start justify-between gap-4 font-bold text-slate-950 marker:content-none">
            ...
            <span
              aria-hidden="true"
              className="flex h-7 w-7 shrink-0 items-center justify-center rounded-full bg-slate-100 text-lg text-slate-700 transition group-open:rotate-45 group-open:bg-blue-600 group-open:text-white"
            >
  ...
  return (
    <section className="px-4 py-12 sm:px-6 sm:py-16">
      <div className="blue-grid relative mx-auto max-w-6xl overflow-hidden rounded-[28px] bg-blue-700 px-5 py-9 text-white shadow-[0_24px_60px_-30px_rgba(29,78,216,0.75)] sm:px-10 sm:py-12 lg:flex lg:items-center lg:justify-between lg:gap-10">
        ...
        <div className="relative mt-7 flex shrink-0 flex-col gap-3 sm:flex-col lg:mt-0">
          <>
            <a
              className="focus-ring inline-flex md:hidden min-h-12 items-center justify-center rounded-full bg-white px-6 font-bold text-blue-800 hover:bg-blue-50"
            ...
            <a
              className="focus-ring hidden md:inline-flex min-h-12 items-center justify-center rounded-full bg-white px-6 font-bold text-blue-800 hover:bg-blue-50"
              href={contactLinks.phone}
            >
